...
public class CustomJwtAuthenticationFilter extends OncePerRequestFilter 
{
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException 
	{
		...
		{
			request.setAttribute("exception", ex);
			LOGGER.error("CustomJwtAuthenticationFilter :	doFilterInternal	:	ERROR" + ex);
			throw ex;
		}
	...
	private String extractJwtFromRequest(HttpServletRequest request) 
	{
		...
		if(null != session.getAttribute("securityKey"))
		{
			bearerToken = (String) session.getAttribute("securityKey");
		}
		else// (StringUtils.hasText(bearerToken) && bearerToken.startsWith("Bearer ")) 
		{
